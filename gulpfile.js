...
var srcPath = 'assets/frontend';
var dataPath = path.join(srcPath, 'templates/_data/');
...
gulp.task('jade', function() {
  ...
  return gulp.src('pages/*.jade', {
      cwd: path.join(srcPath, 'templates')
    })
    .pipe(plumber({
...
gulp.task('img', function() {
  gulp.src(['!svg-sprite', '!svg-sprite/**', '!inline', '!inline/**', '**/*.{jpg,png,svg}'], {
      cwd: path.join(srcPath, 'img')
    })
    .pipe(imagemin({
      progressive: true
    }))
    ...
    .pipe(svg_sprite({
      mode: {
        symbol: {
          ...
          render: {
            styl: {
              dest: path.join(srcPath, 'styl/_global/svg-sprite.styl'),
            }
          }
        }
      },
...
gulp.task('style', function() {
  gulp.src(['style.styl', 'style-ie9.styl'], {
      cwd: path.join(srcPath, 'styl')
    })
    .pipe(plumber({
      errorHandler: notify.onError('Error:  <%= error.message %>')
    }))
...
gulp.task('del', function() {
  return del([path.join(buildPath), path.join(srcPath, 'styl/_global/svg-sprite.styl')]).then(paths => {
...
gulp.task('default', allTasks, function() {
  if (!isOnProduction) {
    ...
    gulp.watch('svg-sprite/*.svg', {
      cwd: path.join(srcPath, 'img')
    }, ['svg', server.reload]);
    gulp.watch(['!svg-sprite', '!svg-sprite/**', '!inline', '!inline/**', '**/*.{jpg,png,svg}'], {
      cwd: path.join(srcPath, 'img')
    }, ['img', server.reload]);
    gulp.watch('**/*{woff,woff2}', {
      cwd: path.join(srcPath, 'fonts')
    }, ['font', server.reload]);
    gulp.watch('**/*.styl', {
      cwd: path.join(srcPath, 'styl')
